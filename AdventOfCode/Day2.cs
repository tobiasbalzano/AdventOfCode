...
namespace AdventOfCode
{
    public class Day2 : Day
    {
        public static void Run()
        {
            Run(2, () =>
            {
                var input = Input2.GetInput().Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries).ToList();

                var parsedInput = input.Select(r => new PasswordRow(r));

                var qualifyingPasswordsVersion1 = parsedInput.Where(p => p.IsValidVer1);
                Console.WriteLine($"{qualifyingPasswordsVersion1.Count()} valid passwords according to first version | {Stopwatch.ElapsedTicks}");

                var qualifyingPasswordsVersion2 = parsedInput.Where(p => p.IsValidVer2);
                Console.WriteLine($"{qualifyingPasswordsVersion2.Count()} valid passwords according to second version | {Stopwatch.ElapsedTicks}");
            });
        }
    }
    internal class PasswordRow
    {
        ...
        public bool IsValidVer1 { get; set; }

        public bool IsValidVer2 { get; set; }

        public PasswordRow(string rowString)
        {
            var values = rowString.Split(" ", StringSplitOptions.RemoveEmptyEntries);
            var charQualifiers = values[0].Split('-');
            From = int.Parse(charQualifiers[0]);
            To = int.Parse(charQualifiers[1]);

            Character = values[1].TrimEnd(':')[0];

            Password = values[2];

            IsValidVer1 = ValidateVersion1();
            IsValidVer2 = ValidateVersion2();
        }
        private bool ValidateVersion1()
        {
            var qualifyingCharCount = Password.Where(c => c.Equals(Character)).Count();
            return qualifyingCharCount >= From && qualifyingCharCount <= To;
        }
        private bool ValidateVersion2()
        {
            int correctCharPositions = 0;
            if (Password[From - 1].Equals(Character))
                correctCharPositions++;
            if (Password[To - 1].Equals(Character))
                correctCharPositions++;

            return correctCharPositions == 1;
        }

    }
    internal static class Input2
    {
        public static string GetInput()
        {
            return @"1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
